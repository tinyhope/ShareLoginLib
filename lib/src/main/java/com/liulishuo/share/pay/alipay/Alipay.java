...
public class Alipay {
    ...
    public static void pay(@NonNull final Activity activity, @Nullable String payData) {
        final Data data = parseData(payData);
        if (data != null) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    PayTask payTask = new PayTask(activity);
                    final Result result = new Result(payTask.payV2(data.toStringData(), true));
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            handlePayResult(result, SsoPayManager.onPayListener);
                        }
                    });
                }
            }).start();
